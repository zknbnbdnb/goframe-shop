...
func (s *sOrder) Add(ctx context.Context, in model.OrderAddInput) (out *model.OrderAddOutput, err error) {
	...
	err = g.DB().Transaction(ctx, func(ctx context.Context, tx gdb.TX) error {
		...
		for _, info := range in.OrderAddGoodsInfos {
			// 商品增加销量
			_, err1 := dao.GoodsInfo.Ctx(ctx).WherePri(info.GoodsId).Increment(dao.GoodsInfo.Columns().Sale, info.Count)
			if err1 != nil {
				return err
			}
			// 商品减少库存
			...
			// 商品规格减少库存
			_, err3 := dao.GoodsOptionsInfo.Ctx(ctx).WherePri(info.GoodsId).Decrement(dao.GoodsOptionsInfo.Columns().Stock, info.Count)
			if err3 != nil {
				return err
			}
